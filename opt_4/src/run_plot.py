# AG REASONING EXECUTION SCRIPT WITH PLOTTING
...
def main(cfg: DictConfig):
    ...
    # Selected DFAs for detailed plotting
    selected_dfa_indices = [8, 9, 10, 11]  # Indices for DFA 9, 10, 11, 12
    
    all_results_without_optimisation = []
    all_results_with_adaptive_optimisation = []

    operational_times_without_optimisation = {i: [] for i in selected_dfa_indices}
    operational_times_with_adaptive_optimisation = {i: [] for i in selected_dfa_indices}
    ...
    for i, (target_dfa, property_dfa) in enumerate(zip(target_dfas, property_dfas)):
        ...
        for _ in range(num_runs):
            ...
            if i in selected_dfa_indices:
                operational_times_without_optimisation[i].append(results_without_optimisation['time_taken'])
                operational_times_with_adaptive_optimisation[i].append(results_with_adaptive_optimisation['time_taken'])
    ...
    dfa_labels = ['DFA 9', 'DFA 10', 'DFA 11', 'DFA 12']

    for idx, i in enumerate(selected_dfa_indices):
        if operational_times_without_optimisation[i] and operational_times_with_adaptive_optimisation[i]:
            plt.figure(figsize=(8, 6))
            plt.plot(range(num_runs), operational_times_without_optimisation[i], label='Without Optimisation', color='blue')
            plt.plot(range(num_runs), operational_times_with_adaptive_optimisation[i], label='With Optimisation', color='green')
            plt.xlabel('Run')
